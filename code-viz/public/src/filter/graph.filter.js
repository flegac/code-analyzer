...
export class GraphFilter {
    ...
    apply(graph) {
        ...
        return Object.fromEntries(Object.entries(graph)
...
GraphFilter.GROUP_REF = '@';
GraphFilter.WILDCARD = '*';
GraphFilter.SEPARATOR = '.';
/* ---------- utilitaires (réutilisés) ---------- */
function compilePatterns(patterns, wildcard) {
    return patterns.map(p => {
        const escaped = p.split(wildcard).map(s => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('.*');
