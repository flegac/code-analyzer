...
class DefaultReader {
    ...
    async readFeatures(projectName, nodeValues) {
        const nodes = {};
...
ProjectService.singleton = new ProjectService();
...
async function loadProject(projectName, fileList = null) {
    ...
    // nodes data
    const nodes = await reader.readFeatures(projectName, nodeValues);
    console.log(`[${projectName}] node data loaded`, nodes);
    const project = new Project(projectName, relation, nodes, config);
