...
export class GraphService {
    ...
    initGraph(container) {
        ...
        this.state.graph = ForceGraph3D()(container);
    ...
    _patchNaNPositions(r = 1000) {
        ...
        this.getGraph().onEngineTick(() => {
            this.getGraph().graphData().nodes.forEach(n => {
                if (isNaN(n.x))
                    n.x = r * (Math.random() - 0.5);
                if (isNaN(n.y))
                    n.y = r * (Math.random() - 0.5);
                if (isNaN(n.z))
                    n.z = r * (Math.random() - 0.5);
            });
        });
    }
    getDiameter() {
    ...
    rebuildGraph() {
        ...
        this.state.nodeById = Object.fromEntries(this.state.nodes.map(n => [n.id, n]));
