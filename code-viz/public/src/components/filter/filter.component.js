...
export class FilterComponent extends BaseComponent {
    ...
    updateState(nodeName, checked) {
        if (checked)
            this.forbiddenNodes.delete(nodeName);
        else
            this.forbiddenNodes.add(nodeName);
    }
    commitState() {
        FF.forbiddenNodes = Array.from(this.forbiddenNodes);
        FF.apply();
    ...
    _initGroupStates() {
        if (!this._checkboxByName || !this._checkboxByName.size)
            return;
...
function updateGroupCheckboxFromChildren(groupPath, ctx) {
    ...
    for (const [leaf, ancestors] of ctx.ancestorsMap.entries()) {
        if (ancestors.includes(groupPath))
            leaves.push(leaf);
    }
    if (leaves.length === 0)
        return;
