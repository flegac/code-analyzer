import { GraphModel, ForceGraph, AdjacencyGraph } from "../project/graph.model.js";
import { MyNode } from "../visuals/graph.service.js";
...
export class CycleCounter extends NodeMetrics {
    ...
    nodes: string[];
    cycles: string[][];
    cycleCount: { [k: string]: number; };
    seenCycles: Set<unknown>;
    ...
    _computeCycles() {
        ...
        for (const scc of sccs) {
            const subgraph: { [k: string]: string[] } = {};
    ...
    _circuit(v: string, start: string, graph: AdjacencyGraph, path: string[], visited: Set<string>, depth: number, maxDepth: number) {
...
function _canonicalCycle(path: string[]) {
...
function _tarjanSCC(graph: { [k: string]: string[] }) {
    let index = 0;
    const indices: { [k: string]: number } = {};
    const lowlink: { [k: string]: number } = {};
    const stack: string[] = [];
