...
import { FF } from "../../filter/filter.service.js";
...
export class FilterComponent extends BaseComponent {
  ...
  private _checkboxByName: Map<any, any>;
  private _updatingLock: boolean;
  ...
  constructor() {
    super({
      ...
      state: {
        ...
        nodeName: null,
        state: FF,
        apply: (e: any) => console.log(e?.target?.value),
        applyPruning: (e: any) => this.applyPruning(e?.target?.value ?? e)
      }
    });
  ...
  updateState(nodeName: string, checked: boolean) {
  ...
  updateGui() {
    ...
    const ctx = {
      ...
      updateState: (n: string, s: boolean) => this.updateState(n, s),
  ...
  _renderGroup(key:string, nodeTree, ctx, depth, pathParts) {
    const item = document.createElement('sl-tree-item');
