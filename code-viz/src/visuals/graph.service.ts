...
class GraphState {
    graph: any | null
    nodeById: { [k: string]: MyNode }
    nodes: MyNode[]
    links: MyLink[]
    selected: MyNode | null
    listeners: Set<(n: MyNode | null) => void>
    constructor() {
        this.graph = null;
        this.nodeById = {};
        this.nodes = [];
...
export class GraphService {
    ...
    _autoResize(container: HTMLElement) {
    ...
    onSelectionChange(cb: (n: MyNode | null) => void) {
    ...
    rebuildGraph() {
        ...
        this.state.nodes = Array.from(nodeIds).map(id => {
            ...
            const node = {
                ...
                y: old?.y,
                z: old?.z,
                read: (label: string) => NN.read(label, id),
                readAll: () => NN.readAll(id),
                write: (label: string, value: any) => NN.write(label, id, value),
            };
