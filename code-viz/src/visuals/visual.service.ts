...
import { G, MyLink, MyNode } from "./graph.service.js";

import { NodeMeshModel } from "../core/mesh/node.mesh.model.js";
...
export class VisualService {
    ...
    textSize(node: MyNode) {
        const isModule = node.read('category') === 'Module';
        return isModule ? 10 / Math.pow(2, node.id.split('.').length) : 1.;
    }

    visibleText(node: MyNode) {
    ...
    rebuildMeshes() {
        ...
        G.getGraph().nodeThreeObject((node: MyNode) => this.getMesh(node, camPosition));
    ...
    updateNodeSizes() {
        ...
        G.state.nodes.forEach(node => {
            ...
            meshes.group.resizeText()
        });
    }

    getLinkColor(link: MyLink) {
    ...
    apply() {
        ...
        graph
            .linkCurvature(.0)
            .linkDirectionalParticles((link: any) => links.particles)
            .linkDirectionalParticleWidth(links.particleWidthMultiplier * links.width)
            .linkDirectionalParticleSpeed(.01)
            .linkWidth((link: any) => link.label === 'hierarchy' ? 10 : links.width)
