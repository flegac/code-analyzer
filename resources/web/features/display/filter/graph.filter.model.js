export class GraphFilter {
    ...
    constructor(config) {
        this.config = config;
        if (config !== null) {
            this.matchSelected = this._prepare(config.selected) ?? (id => true);
            this.matchExcluded = this._prepare(config.excluded) ?? (id => false);
        }
    }
    ...
    _expand(group, visited = new Set()) {
        return group.flatMap(item => {
            ...
            const refGroup = this.config.groups[refName];
            if (!refGroup) return [];
            visited.add(refName);
